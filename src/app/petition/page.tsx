'use client'
// Copied from previous home page to preserve public petition UI
  ...
  return (
    <main className="max-w-2xl mx-auto p-4">
        ...
        <div className="space-y-6 mb-8">
          ...
          {surveys.map(survey => (
            <div key={survey.id} className="p-4 border rounded-lg bg-white">
              <div className="mb-4">
                <div className="font-medium text-gray-900" dangerouslySetInnerHTML={{ __html: survey.title }} />
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          <button className="px-4 py-2 bg-blue-600 text-white rounded disabled:opacity-50 hover:bg-blue-700" disabled={disabledAll} onClick={()=>setOpen(true)}>
            点击签名
          </button>
          {dataUrl && <img src={dataUrl} alt="签名" className="mt-2 w-40 border rounded" />}
        </div>
      </div>
      <button className="w-full py-3 bg-green-600 text-white rounded-lg disabled:opacity-50 hover:bg-green-700 font-medium" disabled={disabledAll} onClick={submit}>
        支持请愿书并提交签名
      </button>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-40">
          <div className="bg-white w-[92%] max-w-xl rounded-lg overflow-hidden">
            ...
            <div className="flex gap-2 justify-end p-3 border-t">
              <button className="px-3 py-2 border rounded hover:bg-gray-50" onClick={()=>setDataUrl('')}>清除</button>
              <button className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" onClick={()=>{ const c = canvasRef.current; if (c) setDataUrl(c.toDataURL('image/png')); setOpen(false); }}>保存签名</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-white w-[92%] max-w-xl rounded-lg overflow-hidden">
            ...
            <div className="flex justify-end gap-2 p-4 border-t">
              ...
              <button className="px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700" onClick={()=>{ localStorage.setItem('consentAccepted','1'); setConsented(true); setConsentOpen(false); }}>我已阅读并同意</button>
            </div>
          </div>
        </div>
      )}
